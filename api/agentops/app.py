...
from .common.environment import API_DOMAIN, APP_DOMAIN, PROTOCOL
...
# Add CORS middleware for the local/LAN dashboards. Credentials are required
# because auth uses an httpOnly session cookie.
api_host = API_DOMAIN.split(":", 1)[0]
dashboard_origins = {
    f"{PROTOCOL}://{APP_DOMAIN}",
    "http://localhost:3000",
    "http://localhost:3001",
    "http://localhost:3002",
    "http://127.0.0.1:3000",
...
logger.info("CORS middleware enabled for dashboards: %s", sorted(dashboard_origins))
