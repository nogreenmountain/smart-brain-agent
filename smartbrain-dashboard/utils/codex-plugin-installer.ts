...
export function buildCodexInstaller({ endpoint, token, bundleUrl }: CodexInstallerOptions): string {
  ...
  const script = [
    ...
    "$mcp=Join-Path $root 'plugins\\company-memory\\.mcp.json'",
    "$config=@{mcpServers=@{'smartbrain-company-memory'=@{type='http';url=$endpoint;bearer_token_env_var='SMARTBRAIN_WIKI_MCP_TOKEN'}}}|ConvertTo-Json -Depth 4",
    '[IO.File]::WriteAllText($mcp,$config,[Text.UTF8Encoding]::new($false))',
    ...
    "Write-Host 'SmartBrain Company Memory installed. Start a new Codex task to use it.' -ForegroundColor Green",
