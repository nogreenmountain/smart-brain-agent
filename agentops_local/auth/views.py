...
def _get_api_domain() -> Optional[str]:
    """
    Removes the port and returns no Domain attribute for host-only IP cookies.
    """
    host = API_DOMAIN.split(':')[0]
    if host == "localhost" or all(part.isdigit() for part in host.split(".")):
        return None
    return host


def _allowed_browser_origins() -> tuple[str, ...]:
    """Origins allowed to call public auth routes in local/LAN deployments."""
    api_host = API_DOMAIN.split(":", 1)[0]
    origins = {
        APP_URL,
        "http://localhost:3000",
        "http://localhost:3001",
        ...
        origins.update(
            {
                ...
                f"http://{api_host}:3001",
                f"http://{api_host}:3002",
            }
        )
...
def _decode_supabase_jwt(token: str) -> SupabaseUserData:
    ...
    We do not verify the signature locally. Instead we call Supabase's
    userinfo endpoint with the token and trust the response. This works
    for both HS256 (anon/service_role keys) and ES256 (login access_tokens)
    without needing to manage JWKS or public keys.
    """
    import os
    import time
    ...
    # SupabaseUserData expects JWT-shape fields. Map userinfo fields to them.
    body.setdefault("sub", body.get("id"))
    body.setdefault("aud", "authenticated")
    body.setdefault("role", body.get("role", "authenticated"))
    body.setdefault("app_metadata", body.get("app_metadata", {}))
    body.setdefault("user_metadata", body.get("user_metadata", {}))
    body.setdefault("iat", int(time.time()))
    body.setdefault("exp", int(time.time()) + 3600)
    body.setdefault("session_id", body.get("id", ""))
    return SupabaseUserData(**body)
...
def _validate_request(request: Request) -> None:
    ...
    if client_ip:
        ...
        logger.error("Request was made to a public route without a client IP address.")
        raise HTTPException(500)

    rate_limit.record_interaction(client_ip)
    if rate_limit.is_blocked(client_ip):
        logger.warning(f"Rate limit exceeded for IP: {client_ip}")
        raise HTTPException(429)

    request_host = request.headers.get("x-forwarded-host") or request.headers.get("host")
    if request_host != API_DOMAIN:
        logger.error(f"Request was made to a public route from an unexpected host: {request_host}")
        raise HTTPException(500)
    ...
    allowed_browser_origins = _allowed_browser_origins()
    if origin and not any(origin.startswith(u) for u in allowed_browser_origins):
    ...
    if referrer and not any(referrer.startswith(u) for u in allowed_browser_origins):
...
async def auth_code(request: Request, code: str) -> RedirectResponse:
    ...
    except (gotrue.errors.AuthApiError, supabase_auth.errors.AuthApiError) as e:
